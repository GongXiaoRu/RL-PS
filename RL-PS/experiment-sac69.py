...
from drl.unpublished.sac1step import Sac1Step
...
class OpfCase(OpfEnv):
    def __init__(self, daily_pv_profile=None,
                 ...
                 voltage_penalty_factor=5.0,  # 电压越限惩罚系数 ρ1
                 multi_objective=True,  # 多目标优化开关
                 voltage_weight=0.0,  # 电压目标权重
                 loss_weight=1.0,  # 网损目标权重
                 # =================================
                 *args, **kwargs):
...
def run_opf_daily(
    ...
    agent = Sac1Step(env, name=f"DailyOPF_VoltageReg_{seed}", seed=seed, path=expdir)
    ...
    # 保存训练数据用于合并绘图
    np.save(os.path.join(image_dir, "train_rewards_SAC.npy"), daily_train_rewards)
    np.save(os.path.join(image_dir, "train_line_loss_SAC.npy"), daily_train_losses)
    np.save(os.path.join(image_dir, "train_voltage_violations_SAC.npy"), daily_voltage_violations)
    return {
...
if __name__ == "__main__":
    ...
    results = run_opf_daily(
        ...
        v_upper=1.05,  # 电压上限
        v_lower=0.95,  # 电压下限
        voltage_penalty_factor=5.0,  # 越限惩罚系数
