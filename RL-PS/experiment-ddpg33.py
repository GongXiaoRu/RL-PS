...
def plot_3d_voltage_heatmap(voltage_matrix, title, save_path, v_upper=1.05, v_lower=0.95):
    """
    生成3D立体热力图，显示33总线网络24小时电压波形

    参数:
        voltage_matrix: 电压矩阵 (24小时 x 33总线)
        title: 图形标题
        save_path: 保存路径
        v_upper: 电压上限
        v_lower: 电压下限
    ...
    buses = np.arange(1, 34)  # 1-33总线

    X, Y = np.meshgrid(hours, buses)

    # 电压矩阵转置以适应网格
    Z = voltage_matrix.T  # 形状应为 (33, 24)
    ...
    ax.set_yticks(np.arange(1, 34, 4))
...
def plot_comparison_3d_voltage_heatmaps(voltage_matrix_no_control, voltage_matrix_control,
    ...
    参数:
        voltage_matrix_no_control: 无控制时的电压矩阵 (24小时 x 33总线)
        voltage_matrix_control: 有控制时的电压矩阵 (24小时 x 33总线)
        best_day: 最佳训练天数
        save_dir: 保存目录
        v_upper: 电压上限
        v_lower: 电压下限
    ...
    ax1.set_yticks(np.arange(1, 34, 4))
    ax1.set_title('(a) Without any control method', fontsize=14, fontweight='bold', pad=15)

    # 添加水平参考平面
    xx1, yy1 = np.meshgrid(np.arange(1, 25, 1), np.arange(1, 34, 1))
    ...
    ax2.set_yticks(np.arange(1, 34, 4))
    ax2.set_title('(b) With the proposed control method', fontsize=14, fontweight='bold', pad=15)

    # 添加水平参考平面
    xx2, yy2 = np.meshgrid(np.arange(1, 25, 1), np.arange(1, 34, 1))
    ...
    # 添加主标题
    fig.suptitle(f'24-hour bus voltage profiles in 33-bus network (Best Episode {best_day})',
...
def run_opf_daily(
            ...
            if voltage_regulation_enabled and hourly_info:
                ...
                for hour_info in hourly_info:
                    if hour_info.get("voltage_info"):
                        ...
                        voltage_matrix_control.append([np.nan] * 33)
                voltage_matrix_control_best = np.array(voltage_matrix_control)  # 24x33
                ...
                for hour_info in hourly_info_no_control:
                    if hour_info.get("voltage_info"):
                        ...
                        voltage_matrix_no_control.append([np.nan] * 33)
                voltage_matrix_no_control_best = np.array(voltage_matrix_no_control)  # 24x33
    ...
    if len(daily_train_rewards) >= convergence_window:
        ...
        if len(convergence_indices) > 0:
            first_convergence = convergence_indices[0] + convergence_window  # 加上窗口偏移得到实际回合数
            print(f"Convergence episode (first time {convergence_window}-episode moving avg reaches {convergence_threshold*100:.0f}% of final avg): {first_convergence}")
        else:
    ...
    if voltage_regulation_enabled and voltage_matrix_no_control_best is not None and voltage_matrix_control_best is not None:
        ...
        plot_3d_voltage_heatmap(
            ...
            "24-hour bus voltage profiles in 33-bus network (Without any control method)",
        ...
        plot_3d_voltage_heatmap(
            ...
            "24-hour bus voltage profiles in 33-bus network (With the proposed control method)",
    ...
    return {
        ...
        "convergence_episode": first_convergence if len(convergence_indices) > 0 else None,
