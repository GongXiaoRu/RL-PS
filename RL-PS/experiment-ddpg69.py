...
class OpfCase(OpfEnv):
    ...
    def _define_opf(self):
        ...
        Bus = np.array([
            ...
            [6, 2.6, 2.2], [7, 40.4, 30], [8, 75, 54], [9, 30, 22], [10, 28, 19],
            [11, 145, 104], [12, 145, 104], [13, 8, 5.5], [14, 8, 5.5], [15, 0, 0],
            [16, 45.5, 30], [17, 60, 35], [18, 60, 35], [19, 0, 0], [20, 1, 0.6],
            [21, 114, 81], [22, 5.3, 3.5], [23, 0, 0], [24, 28, 20], [25, 0, 0],
            [26, 0, 0], [27, 14, 10], [28, 14, 10], [29, 26, 18.6], [30, 26, 18.6],
            [31, 0, 0], [32, 0, 0], [33, 14, 10], [34, 19.5, 14], [35, 6, 4],
            [36, 26, 18.6], [37, 26, 18.6], [38, 0, 0], [39, 24, 17], [40, 24, 17],
            [41, 1.2, 1], [42, 0, 0], [43, 6, 4.3], [44, 0, 0], [45, 39.22, 26.3],
            [46, 39.22, 26.3], [47, 0, 0], [48, 79, 56.4], [49, 384.7, 274.5], [50, 384.7, 274.5],
            [51, 40.5, 28.3], [52, 3.6, 2.7], [53, 4.35, 3.5], [54, 26.4, 19], [55, 24, 17.2],
            [56, 0, 0], [57, 0, 0], [58, 0, 0], [59, 100, 72], [60, 0, 0],
            [61, 1244, 888], [62, 32, 23], [63, 0, 0], [64, 227, 162], [65, 59, 42],
            [66, 18, 13], [67, 18, 13], [68, 28, 20], [69, 28, 20]
        ])
        ...
        # 创建光伏节点 - 69节点系统配置
        pv_nodes = [61, 62, 63]  # 节点62, 63, 64 (Python索引61, 62, 63)
        ...
        # 创建储能 - 配置在节点64 (Python索引63)
        storage_bus = 63
        storage_capacity = 20  # MWh
        storage_power = 2  # MW
...
def plot_3d_voltage_heatmap(voltage_matrix, title, save_path, v_upper=1.05, v_lower=0.95):
    """
    生成3D立体热力图，显示69总线网络24小时电压波形

    参数:
        voltage_matrix: 电压矩阵 (24小时 x 69总线)
        title: 图形标题
        save_path: 保存路径
        v_upper: 电压上限
        v_lower: 电压下限
    ...
    buses = np.arange(1, 70)  # 1-69总线

    X, Y = np.meshgrid(hours, buses)

    # 电压矩阵转置以适应网格
    Z = voltage_matrix.T  # 形状应为 (69, 24)
    ...
    ax.set_yticks(np.arange(1, 70, 8))
...
def plot_comparison_3d_voltage_heatmaps(voltage_matrix_no_control, voltage_matrix_control,
    ...
    参数:
        voltage_matrix_no_control: 无控制时的电压矩阵 (24小时 x 69总线)
        voltage_matrix_control: 有控制时的电压矩阵 (24小时 x 69总线)
        best_day: 最佳训练天数
        save_dir: 保存目录
        v_upper: 电压上限
        v_lower: 电压下限
    ...
    ax1.set_yticks(np.arange(1, 70, 8))
    ax1.set_title('(a) Without any control method', fontsize=14, fontweight='bold', pad=15)

    # 添加水平参考平面
    xx1, yy1 = np.meshgrid(np.arange(1, 25, 1), np.arange(1, 70, 1))
    ...
    ax2.set_yticks(np.arange(1, 70, 8))
    ax2.set_title('(b) With the proposed control method', fontsize=14, fontweight='bold', pad=15)

    # 添加水平参考平面
    xx2, yy2 = np.meshgrid(np.arange(1, 25, 1), np.arange(1, 70, 1))
    ...
    # 添加主标题
    fig.suptitle(f'24-hour bus voltage profiles in 69-bus network (Best Episode {best_day})',
...
def run_opf_daily(
    ...
    参数:
        ...
        loss_weight: 网损目标权重
        convergence_window: 收敛检测滑动窗口大小
        convergence_threshold: 收敛阈值比例（相对于最终平均奖励）
    """
    ...
    agent = Ddpg1Step(env, name=f"DailyOPF_VoltageReg_{seed}", seed=seed, path=expdir)
            ...
            if voltage_regulation_enabled and hourly_info:
                ...
                for hour_info in hourly_info:
                    if hour_info.get("voltage_info"):
                        ...
                        voltage_matrix_control.append([np.nan] * 69)
                voltage_matrix_control_best = np.array(voltage_matrix_control)  # 24x69
                ...
                for hour_info in hourly_info_no_control:
                    if hour_info.get("voltage_info"):
                        ...
                        voltage_matrix_no_control.append([np.nan] * 69)
                voltage_matrix_no_control_best = np.array(voltage_matrix_no_control)  # 24x69
    ...
    if len(daily_train_rewards) >= convergence_window:
        ...
        if len(convergence_indices) > 0:
            convergence_episode = convergence_indices[0] + convergence_window  # 加上窗口偏移得到实际回合数
            print(f"Convergence episode (first time {convergence_window}-episode moving avg reaches {convergence_threshold*100:.0f}% of final avg): {convergence_episode}")
        else:
    ...
    if voltage_regulation_enabled and voltage_matrix_no_control_best is not None and voltage_matrix_control_best is not None:
        ...
        plot_3d_voltage_heatmap(
            ...
            "24-hour bus voltage profiles in 69-bus network (Without any control method)",
        ...
        plot_3d_voltage_heatmap(
            ...
            "24-hour bus voltage profiles in 69-bus network (With the proposed control method)",
    ...
    # 保存训练数据用于合并绘图
    np.save(os.path.join(image_dir, "train_rewards_DDPG.npy"), daily_train_rewards)
    np.save(os.path.join(image_dir, "train_line_loss_DDPG.npy"), daily_train_losses)
    np.save(os.path.join(image_dir, "train_voltage_violations_DDPG.npy"), daily_voltage_violations)
    return {
